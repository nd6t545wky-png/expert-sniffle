...
describe("arm retention in the readiness score", () => {
  const base = {
    sleepHours: 8, sleepQuality: 4, energy: 4, mood: 4, stress: 2,
    shoulder: 0, elbow: 0, forearm: 0, lat: 0, lower: 0,
    illness: "no" as const, warningSigns: "no" as const,
    previousSessionResponse: "as_expected" as const,
  };

  it("leaves a good retention alone", () => {
    const result = computeReadiness({ ...base, armRetentionPct: 95 });
    expect(result.planLevel).toBe("full");
    expect(result.signals.some((s) => s.type === "arm_retention")).toBe(false);
  });

  it("reduces the day when strength was not held", () => {
    const result = computeReadiness({ ...base, armRetentionPct: 84 });
    expect(result.signals.find((s) => s.type === "arm_retention")?.severity).toBe("moderate");
    expect(result.planLevel).toBe("reduced");
  });

  it("drops to recovery when more than a fifth was lost", () => {
    const result = computeReadiness({ ...base, armRetentionPct: 74 });
    expect(result.signals.find((s) => s.type === "arm_retention")?.severity).toBe("high");
    expect(result.planLevel).toBe("recovery");
  });

  it("says what it saw, in the reasons", () => {
    const result = computeReadiness({ ...base, armRetentionPct: 84 });
    expect(result.reasons.some((r) => /84% of pre-outing strength/.test(r))).toBe(true);
  });

  it("ignores the field when no retention was measured", () => {
    // Absent must never read as zero — that would hold every session.
    expect(computeReadiness(base).planLevel).toBe("full");
    expect(computeReadiness({ ...base, armRetentionPct: 0 }).planLevel).toBe("full");
  });
});
