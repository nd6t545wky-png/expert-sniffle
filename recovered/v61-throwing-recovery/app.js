const Domain = window.PitchingDomain;
if (!Domain) throw new Error("The Pitching OS domain module did not load");
const STORAGE_KEY = Domain.STORAGE_KEY_V2;
...
const ANNUAL_START = Domain.ANNUAL_START;
const SUMMER_FIRST_GAME = Domain.COOMERA_OPENER_DATE;
...
const PHASE_COLORS = {
  winter_ball: "#e52b21",
  transition: "#149ca5",
  velocity_development: "#149ca5",
  preseason: "#5b2e91",
  summer_season: "#5b2e91"
};

const PHASES = Domain.PROGRAMME_PHASES.map((phase) => ({
  ...phase,
...
function trapBarPrescription(fallback, pbs) {
  const trainingMax = pbs?.trainingMaxes?.lifts?.trapBarDeadlift;
  const match = String(fallback).match(/Trap bar deadlift\s+(\d+)\s*×\s*(\d+)\s*@\s*([\d.]+)\s*kg/i);
  if (!match || !Number(trainingMax?.value)) return fallback;
  const [, sets, reps, approvedLoad] = match;
  const percent = round(Number(approvedLoad) / 150 * 100, 1);
...
function phaseForWeek(week) {
  const approved = Domain.getPhaseForWeek(week);
  return PHASES.find((phase) => phase.id === approved.id);
}

function isSummerCompetitionPhase(phaseId) {
  return phaseId === "summer_season";
}

function isTransitionPhase(phaseId) {
  return phaseId === "transition";
}

function isWinterCompetitionPhase(phaseId) {
  return phaseId === "winter_ball";
}

function fncRoundForRange(start, end) {
  const startIso = isoDate(start);
  const endIso = isoDate(end);
  const event = Domain.getCalendarEvents().find((item) => item.team === "Norths" && item.type === "game" && item.date >= startIso && item.date <= endIso);
  if (!event) return null;
  const round = Number(event.id.match(/-r(\d+)$/)?.[1]);
  return { round, date: event.date };
}

function getWeekPlan(week, pbs = null) {
  const approved = Domain.getAnnualWeek(week);
  ...
  return {
    ...
    focus: approved.focus,
    mondayLift: trapBarPrescription(approved.mondayLift, pbs),
    throwing: approved.throwing,
    recovery: approved.recovery,
    competition,
    scheduleConfirmed: Boolean(fncRound),
    scheduleStatus,
    scheduleTone,
    theme: `${phase.name} · ${approved.focus}`
  };
}
...
function plannedDayLabel(phaseId, day) {
  ...
  if (phaseId === "preseason" || phaseId === "velocity_development") return [
...
function weeklyRhythmText(phaseId) {
  ...
  if (phaseId === "velocity_development") return "Long toss and velocity development · No league game assumed";
...
function seasonThemeForPhase(phaseId) {
  return ["winter_ball", "transition", "velocity_development", "preseason"].includes(phaseId) ? "norths" : "coomera";
}
...
function plannedStressShape(phaseId) {
  ...
  if (phaseId === "preseason" || phaseId === "velocity_development") return [55, 55, 90, 25, 35, 70, 0];
...
function loadState() {
  try {
    return hydrateState(Domain.migrateLocalStorage(localStorage, initialState).state);
  } catch (error) {
    console.error("Pitching OS state could not be loaded safely", error);
    return initialState();
  }
}
...
function hydrateState(stored, defaults = initialState()) {
  ...
  const hasEstablishedWorkspace = Boolean(
    (stored.profile?.name && stored.profile.name !== "Athlete")
    || stored.profile?.photoDataUrl
    || Number(stored.profile?.height) > 0
    || Number(stored.profile?.weight) > 0
    || Object.keys(stored.sessions || {}).length
    || Object.keys(stored.pre || {}).length
    || Object.keys(stored.post || {}).length
    || Object.keys(stored.healthPrefill || {}).length
    || Object.keys(stored.pulseImports || {}).length
    || Object.keys(stored.bullpens || {}).length
    || Object.keys(stored.weeklyReviews || {}).length
    || Object.values(stored.trainingHistory || {}).some((events) => Array.isArray(events) && events.length)
    || (Array.isArray(stored.mechanics?.assessments) && stored.mechanics.assessments.length)
    || Object.keys(stored.nutrition?.meals || {}).length
    || Object.keys(stored.nutrition?.hydration || {}).length
  );
  ...
  return {
    ...defaults,
    ...stored,
    onboardingComplete: Boolean(stored.onboardingComplete || hasEstablishedWorkspace),
...
const savedPendingChanges = storedArray(SYNC_PENDING_STORAGE);
const cloudSync = {
  key: savedSyncKey,
  ...
  revision: 0,
  ready: false,
  dirty: savedPendingChanges.length > 0,
  timer: null,
  inFlight: null,
...
async function decryptCloudSnapshot(payload, syncKey) {
  ...
  if (parsed?.version === 2) return Domain.migrateV2State(parsed).state;
  if (parsed?.version === 1) {
    const defaults = initialState();
    return Domain.migrateV1State({
      ...defaults,
      ...parsed,
      version: 1,
      completedTasks: parsed.completedTasks || {},
      skippedTasks: parsed.skippedTasks || {},
      taskCompletionUpdatedAt: parsed.taskCompletionUpdatedAt || {},
      trainingHistory: parsed.trainingHistory || HistoryDomain.emptyTrainingHistory()
    }).state;
  }
...
function mergeDatedSessions(remoteSessions = {}, localSessions = {}) {
  const output = {};
  for (const date of new Set([...Object.keys(remoteSessions || {}), ...Object.keys(localSessions || {})])) {
    const remote = remoteSessions?.[date];
    const local = localSessions?.[date];
    if (!remote) { output[date] = local; continue; }
    if (!local) { output[date] = remote; continue; }
    if (Number(remote.planRevision) !== Number(local.planRevision)) {
      output[date] = Number(local.planRevision) > Number(remote.planRevision) ? local : remote;
      continue;
    }
    const samePlan = remote.plan?.tasks?.length === local.plan?.tasks?.length
      && remote.plan.tasks.every((task, index) => task.id === local.plan.tasks[index]?.id);
    if (!samePlan) {
      output[date] = local;
      continue;
    }
    const taskStates = {};
    for (const taskId of Object.keys(local.taskStates || {})) {
      const remoteState = remote.taskStates?.[taskId];
      const localState = local.taskStates?.[taskId];
      const remoteTime = Date.parse(remoteState?.updatedAt || "") || 0;
      const localTime = Date.parse(localState?.updatedAt || "") || 0;
      taskStates[taskId] = localTime >= remoteTime ? localState : remoteState;
    }
    output[date] = { ...local, taskStates };
  }
  return output;
}
...
function mergeProfiles(remoteProfile = {}, localProfile = {}, remoteFallback = 0, localFallback = 0) {
  const remoteUpdated = Date.parse(remoteProfile.updatedAt || "") || 0;
  const localUpdated = Date.parse(localProfile.updatedAt || "") || 0;
  if (remoteUpdated || localUpdated) return localUpdated >= remoteUpdated ? localProfile : remoteProfile;
  const defaults = initialState().profile;
  const laterIsLocal = localFallback >= remoteFallback;
  return Object.fromEntries([...new Set([...Object.keys(defaults), ...Object.keys(remoteProfile), ...Object.keys(localProfile)])]
    .filter((key) => key !== "updatedAt")
    .map((key) => {
      const remoteValue = remoteProfile[key];
      const localValue = localProfile[key];
      const remoteMeaningful = remoteValue !== undefined && remoteValue !== defaults[key];
      const localMeaningful = localValue !== undefined && localValue !== defaults[key];
      if (remoteMeaningful !== localMeaningful) return [key, remoteMeaningful ? remoteValue : localValue];
      return [key, laterIsLocal ? localValue : remoteValue];
    }));
}
...
function mergeCloudStates(remoteState, localState) {
  ...
  return {
    ...
    ...latest,
    version: 2,
    sessions: mergeDatedSessions(remoteState?.sessions, localState?.sessions),
    ...
    profile: mergeProfiles(remoteState?.profile, localState?.profile, remoteTime, localTime),
    nutrition: {
...
function refreshSubmittedHealthCheck(date) {
  ...
  const programme = Domain.programmeWeekForDate(date);
  assignDatedSession(date, "plan_reassigned_after_health_refresh", getWeekPlan(programme.week, state.pbs), programme.dayIndex);
}
...
async function pushCloudState() {
  ...
  cloudSync.inFlight = (async () => {
    ...
    try {
      ...
      console.warn("Training history upload will retry", historyError);
      setCloudStatus("synced", "Encrypted autosave is current; training history will retry automatically");
    }
  })().catch((error) => {
...
async function reconcileCloudSync(options = {}) {
  ...
  persistSyncJournal();
  cloudSync.ready = true;
  if (!options.preferRemote && cloudSync.dirty) {
    const navigation = { page: state.page, selectedWeek: state.selectedWeek, selectedDay: state.selectedDay, lastOpenDate: state.lastOpenDate };
    const trainingHistory = state.trainingHistory;
    state = { ...hydrateState(mergeCloudStates(remote.data, cloudSnapshot())), ...navigation, trainingHistory };
    localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
  ...
  try {
    ...
    console.warn("Training history sync will retry", historyError);
    setCloudStatus("synced", "Encrypted autosave is current; training history will retry automatically");
  }
}
...
async function initializeCloudSync() {
  ...
  try {
    ...
    console.error("Pitching OS cloud state could not be loaded safely", error);
    cloudSync.ready = true;
    setCloudStatus(navigator.onLine ? "error" : "offline", navigator.onLine ? error.message : "Saved locally; cloud will retry when online");
  }
}
...
function throwingRampTask(prefix, options = {}) {
  const optional = options.optional === true;
  const game = options.game === true;
  return task(`${prefix}-regulation-ramp`, 2, "Throwing Entry", "Confirm pain-free rhythm with a regulation baseball before any constraint throws.", "Regulation-ball entry ramp", `${optional ? "Optional · " : ""}8–12 throws · 30–60 ft · 40–50% perceived intent · count these throws in the listed session total`, game ? "Start easy even on game day. The warm-up earns higher intent; the calendar does not." : "Use an easy arc and relaxed grip. Advance only when each distance feels normal.", {
    setup: "Complete the temperature, mobility and cuff/scap preparation first. Use a regulation baseball and a relaxed catch partner.",
    execution: "Begin at 30–45 ft, then move toward 60 ft only if rhythm, direction and symptoms are normal. This is a load ramp, not a test or an injury screen.",
    rest: "Natural catch-play rhythm; take extra time between throws if needed.",
    stop: "Do not begin plyo-ball work, long toss, pulldowns or mound throwing if pain appears, pain increases, or the arm does not loosen. Use the health-hold pathway instead."
  });
}

function plyoTask(prefix, index, name, prescription, intent, cue) {
  return task(`${prefix}-plyo-${index}`, 2, "Plyo Ball Constraints", "Low-volume constraint throws; no pivot pickoffs. These are not proven to acutely increase velocity or prevent injury.", name, `${prescription} · ${intent}% perceived intent`, cue, {
    ...
    stop: "Stop for pain, loss of arm path, or a clear drop in movement quality. Evidence for compliant plyo balls is limited; they are not a substitute for a coached return-to-throw plan."
  });
}
...
const RECOVERY_STAGE = 6;

// datedTaskId strips one leading "-word" segment from a task ID, so recovery IDs
// carry two tokens ("recovery-throw", "recovery-gym"). A single token would
// collapse `${p}-recovery-heat` onto the warm-up task `${p}-heat` and produce
// duplicate IDs, which state-schema rejects on load.
function modalityTask(prefix, scope, modality, stageDescription) {
  const name = modality.optional ? `${modality.name} (optional)` : modality.name;
  return task(`${prefix}-recovery-${scope}-${modality.id}`, RECOVERY_STAGE, "Recover", stageDescription, name, modality.prescription, modality.evidence, {
    setup: `Window: ${modality.window}.`,
    execution: modality.optional
      ? "Optional. Skip it without penalty; the protocol does not depend on it."
      : "Complete as prescribed. This is part of the core protocol.",
    rest: "Once per session unless the window says otherwise.",
    stop: "Stop and record symptoms in the post-session questionnaire if anything worsens."
  });
}

function laterDaysTask(prefix, plan) {
  const later = plan.days.filter((item) => item.dayOffset > 0);
  if (!later.length) return [];
  const summary = later.map((item) => `${item.title}: ${item.modalities.map((m) => m.name).join(" · ")}`).join("  ||  ");
  return [
    task(`${prefix}-recovery-throw-ahead`, RECOVERY_STAGE, "Recover", "The protocol continues past today.", "Days ahead in this recovery block", summary, "Rotation peak force is lowest immediately after throwing and highest at day 5, so recovery runs as a five-day curve (Pexa 2025).", {
      setup: `Tier: ${plan.tier}. Runs ${plan.days.length} days from today.`,
      execution: later.flatMap((item) => item.notes).join(" "),
      rest: "Review at the start of each of the next days.",
      stop: "If pain rises rather than falls across these days, stop and seek review."
    })
  ];
}

function recoveryTasks(prefix, game = false) {
  const tier = Domain.classifyThrowingLoadTier(game ? { gamePitches: 75 } : { totalThrows: 45, intentPercent: 60 });
  const plan = Domain.buildThrowingRecoveryPlan(tier);
  const today = plan.days[0];
  return [
    ...today.modalities.map((modality) => modalityTask(prefix, "throw", modality, today.focus)),
    ...laterDaysTask(prefix, plan)
  ];
}

function gymRecoveryTasks(prefix, sessionType = "hypertrophy") {
  const plan = Domain.buildGymRecoveryPlan(sessionType);
  const today = plan.days[0];
  return [
    ...today.modalities.map((modality) => modalityTask(prefix, "gym", modality, today.focus)),
    task(`${prefix}-recovery-gym-ahead`, RECOVERY_STAGE, "Recover", "The gym block has a next-day component.", "Tomorrow after this lift", plan.days[1].modalities.map((m) => m.name).join(" · "), plan.days[1].notes.join(" "), {
      setup: `Session type: ${sessionType.replace("_", " ")}.`,
      execution: "Complete tomorrow, not today.",
      rest: "Once.",
      stop: "Skip if symptoms are worse than at the end of the lift."
    })
  ];
}
...
const MECHANICS_INTERVENTIONS = {
  ...
  deceleration: {
    ...
    drill: "Rocker throw — controlled finish",
    drillDose: "1 × 4 · 225 g · 55–65% perceived intent",
    drillCue: "Use a smooth arm path and balanced finish. Do not force layback or treat this as a deceleration test.",
    gymTarget: "row",
  ...
  if (day === 0) {
    return {
      ...
      tasks: [
        ...
        throwingRampTask(p),
        plyoTask(p, 2, "Plyo Ball Reverse Throw — 1,000 g", "1 × 4", 45, "Submaximal patterning only; do not turn the heavy implement into a velocity throw."),
        plyoTask(p, 3, "Plyo Ball Roll-In Throw — 450 g", "1 × 5", 55, "Organize the lead leg and throw through the target."),
        plyoTask(p, 4, "Plyo Ball Rocker Throw — 225 g", "1 × 4", 55, "Use rhythm; do not chase ball speed on a recovery day."),
        ...
        ...armCare(p),
        ...recoveryTasks(p),
        ...gymRecoveryTasks(p, "max_strength")
  ...
  if (day === 1) {
    return {
      ...
      tasks: [
        ...
        throwingRampTask(p),
        plyoTask(p, 2, "Plyo Ball Roll-In Throw — 450 g", "1 × 5", 60, "Lead-leg stability before arm acceleration."),
        plyoTask(p, 3, "Plyo Ball Rocker Throw — 225 g", "1 × 4", 65, "Direct energy through the target."),
        plyoTask(p, 4, "Plyo Ball Walking Windup — 150 g", "1 × 4", 65, "Rhythm and direction; no velocity chase."),
  ...
  if (day === 2) {
    return {
      ...
      tasks: [
        ...
        throwingRampTask(p),
        plyoTask(p, 2, "Plyo Ball Roll-In Throw — 450 g", "1 × 4", 65, "Fast lower half with a connected arm; the 450 g ball remains submaximal."),
        plyoTask(p, 3, "Plyo Ball Rocker Throw — 225 g", "1 × 4", 75, "Firm direction and full deceleration."),
        plyoTask(p, 4, "Plyo Ball Walking Windup — 150 g", "1 × 3", 80, "Accelerate only while positions stay organized."),
        plyoTask(p, 5, "Plyo Ball Walking Windup — 125 g", "1 × 3", 85, "Fast but controlled; stop before timing gets early."),
        plyoTask(p, 6, "Plyo Ball Walking Windup — 100 g", "1 × 2", 85, "The lightest ball is a speed exposure, not permission to lose direction."),
  ...
  if (day === 3) {
    return {
      ...
      tasks: [
        ...basePrep(p, "recovery throwing"),
        throwingRampTask(p),
        plyoTask(p, 1, "Plyo Ball Reverse Throw — 1,000 g", "1 × 4", 45, "Easy submaximal patterning and full exhale."),
        plyoTask(p, 2, "Plyo Ball Roll-In Throw — 450 g", "1 × 4", 50, "Quiet effort; clean lead-leg organization."),
        plyoTask(p, 3, "Plyo Ball Rocker Throw — 225 g", "1 × 3", 50, "Finish balanced and loose."),
  ...
  if (day === 4) {
    return {
      ...
      tasks: [
        ...
        throwingRampTask(p),
        plyoTask(p, 2, "Plyo Ball Roll-In Throw — 450 g", "1 × 3", 55, "Crisp lead-leg block without max intent."),
        plyoTask(p, 3, "Plyo Ball Rocker Throw — 225 g", "1 × 3", 60, "Finish loose and directional."),
        plyoTask(p, 4, "Plyo Ball Walking Windup — 150 g", "1 × 2", 60, "Two clean rhythm throws; stop there."),
  ...
  if (day === 5) {
    return {
      ...
      tasks: [
        ...
        throwingRampTask(p, { game: true }),
        plyoTask(p, 3, "Plyo Ball Rocker Throw — 225 g", "1 × 3", 60, "Crisp, familiar patterning without fatigue."),
        plyoTask(p, 4, "Plyo Ball Walking Windup — 150 g", "1 × 2", 65, "Two rhythm throws; weighted-ball warm-up has not been shown to raise subsequent regulation-ball velocity."),
...
function summerSession(week, day) {
  ...
  if (day === 0) {
    return {
      ...
      tasks: [
        ...
        throwingRampTask(p, { optional: true }),
        plyoTask(p, 2, "Plyo Ball Roll-In Throw — 450 g", "1 × 3", 45, "Optional familiar patterning only; omit it if complete rest is the better recovery choice."),
        plyoTask(p, 3, "Plyo Ball Rocker Throw — 225 g", "1 × 3", 45, "Easy rhythm; no underload balls today."),
  ...
  if (day === 1 || day === 3) {
    ...
    return {
      ...
      tasks: [
        ...
        throwingRampTask(p),
        plyoTask(p, 2, "Plyo Ball Roll-In Throw — 450 g", isThursday ? "1 × 3" : "1 × 5", isThursday ? 50 : 60, "Organize the lead leg and stay directional."),
        plyoTask(p, 3, "Plyo Ball Rocker Throw — 225 g", isThursday ? "1 × 3" : "1 × 4", isThursday ? 55 : 65, "Finish under control."),
        plyoTask(p, 4, "Plyo Ball Walking Windup — 150 g", isThursday ? "1 × 2" : "1 × 4", isThursday ? 55 : 65, "Rhythm only; no radar chase."),
  ...
  if (day === 2) {
    return {
      ...
      tasks: [
        ...
        ...armCare(p, "low"),
        ...recoveryTasks(p),
        ...gymRecoveryTasks(p, "hypertrophy")
  ...
  if (day === 4 || day === 6) {
    ...
    return {
      ...
      tasks: [
        ...
        throwingRampTask(p, { game: true }),
        plyoTask(p, 3, "Plyo Ball Rocker Throw — 225 g", "1 × 3", 60, "Use familiar, crisp game-day rhythm."),
        plyoTask(p, 4, "Plyo Ball Walking Windup — 150 g", "1 × 2", 65, "Stop before fatigue; do not chase an acute velocity effect."),
  ...
  if (appearedFriday) {
    return {
      ...
      tasks: [
        ...basePrep(p, "recovery"),
        throwingRampTask(p, { optional: true }),
        plyoTask(p, 2, "Plyo Ball Roll-In Throw — 450 g", "1 × 3", 45, "Optional only if symptom-free and the arm improves during the regulation-ball ramp."),
  ...
  return {
    ...
    tasks: [
      ...
      throwingRampTask(p),
      plyoTask(p, 2, "Plyo Ball Roll-In Throw — 450 g", "1 × 3", 50, "Easy rhythm."),
      plyoTask(p, 3, "Plyo Ball Rocker Throw — 225 g", "1 × 3", 55, "Crisp and controlled."),
...
function transitionWednesdaySession(week) {
  ...
  return {
    ...
    tasks: [
      ...
      throwingRampTask(p),
      plyoTask(p, 2, "Plyo Ball Roll-In Throw — 450 g", "1 × 4", 50, "Smooth patterning only."),
      plyoTask(p, 3, "Plyo Ball Rocker Throw — 225 g", "1 × 3", 55, "Stay directional and finish balanced."),
...
function nonCompetitionSaturdaySession(week) {
  ...
  return {
    ...
    tasks: [
      ...
      throwingRampTask(p, { optional: transition }),
      plyoTask(p, 2, "Plyo Ball Roll-In Throw — 450 g", "1 × 4", transition ? 45 : 60, "Smooth rhythm and a balanced finish."),
      plyoTask(p, 3, "Plyo Ball Rocker Throw — 225 g", "1 × 3", transition ? 50 : 65, "Organize the lead leg without chasing speed."),
...
function adaptTaskForReadiness(item, planLevel, gameDay) {
  ...
  if (type === "plyo") {
    adapted.prescription = planLevel === "recovery"
      ? "Omitted today by the readiness adjustment"
...
function buildLiveSession(week = selectedWeekPlan(), day = state.selectedDay) {
  ...
  if (state.pre[date]?.safety?.override || state.pre[date]?.risk === "red") return recoveryOnlySession(week, day);
  const calendarEvent = Domain.calendarEventForDate(date);
  const coomeraEventThisWeek = Array.from({ length: 7 }, (_, index) => Domain.calendarEventForDate(isoDate(addDays(week.start, index))))
    .some((event) => event?.team === "Coomera Cubs");
  let session;
  if (calendarEvent?.team === "Coomera Cubs" || isSummerCompetitionPhase(week.phase.id)) session = summerSession(week, day);
  else if (coomeraEventThisWeek && day === 5) session = summerSession(week, day);
  else if (isTransitionPhase(week.phase.id) && day === 2) session = transitionWednesdaySession(week);
  else if (["transition", "velocity_development", "preseason"].includes(week.phase.id) && day === 5) session = nonCompetitionSaturdaySession(week);
...
function getSession(week = selectedWeekPlan(), day = state.selectedDay) {
  const date = isoDate(addDays(week.start, day));
  return state.sessions?.[date]?.plan || buildLiveSession(week, day);
}

function latestPlanRevision(date) {
  const activeRevision = Number(state.sessions?.[date]?.planRevision || 0);
  const historyRevision = (state.trainingHistory?.events || [])
    .filter((event) => event.collection === "planSnapshots" && event.date === date)
    .reduce((highest, event) => Math.max(highest, Number(event.payload?.planRevision || 0)), 0);
  return Math.max(activeRevision, historyRevision);
}

function assignDatedSession(date, reason, week = selectedWeekPlan(), day = state.selectedDay) {
  const entity = Domain.createDatedSession(date, latestPlanRevision(date) + 1, buildLiveSession(week, day));
  state.sessions = { ...(state.sessions || {}), [date]: entity };
  capturePlanSnapshot(date, reason, entity);
  return entity;
}

function capturePlanSnapshot(date, reason, entity = state.sessions?.[date]) {
  if (!entity) return null;
  return appendTrainingHistory("planSnapshots", date, reason, {
    date,
    datedSessionId: entity.id,
    planRevision: entity.planRevision,
    week: entity.programmeWeek,
    day: entity.dayIndex,
    phaseId: entity.phaseId,
    calendarEventId: entity.calendarEventId,
    planSource: "pitching_os",
    readinessPlanLevel: state.pre[date]?.planLevel || "unsubmitted",
    session: JSON.parse(JSON.stringify(entity.plan))
  });
}
...
function mostRecentPainBefore(date) {
  const candidates = Object.keys(state.pre || {})
    .filter((candidate) => candidate < date)
    .sort((left, right) => right.localeCompare(left));
  for (const candidate of candidates) {
    const pre = state.pre?.[candidate];
    if (pre && Number.isFinite(Number(pre.shoulder)) && Number.isFinite(Number(pre.elbow))) {
      return { shoulder: Number(pre.shoulder), elbow: Number(pre.elbow) };
    }
  }
  return undefined;
}
...
function calculateReadiness(values, date) {
  ...
  const restingHeartRateBaseline = personalMetricBaseline(date, "restingHeartRate", restingHeartRateSource);
  const subjectiveBaselines = subjectiveBaselinesForDate(date);
  const result = Domain.evaluateReadiness({
    ...values,
    ...
    elbow: Number(values.elbow),
    forearm: Number(values.forearm),
    lat: Number(values.lat),
    lower: Number(values.lower)
  }, {
    previousPain: mostRecentPainBefore(date),
    hrv: values.hrvMs,
  ...
  return {
    ...
    sleepSource,
    baselines: {
      hrv: hrvBaseline.count >= 5 ? round(hrvBaseline.value, 1) : null,
      restingHeartRate: restingHeartRateBaseline.count >= 5 ? round(restingHeartRateBaseline.value, 1) : null,
      hrvSamples: hrvBaseline.count,
      restingHeartRateSamples: restingHeartRateBaseline.count,
...
function completedForDate(date, session) {
  const entity = state.sessions?.[date];
  if (!entity) return { count: 0, resolved: 0, performed: 0, completed: 0, modified: 0, skipped: 0, notAttempted: session.tasks.length, total: session.tasks.length };
  const progress = Domain.taskProgress(entity.taskStates);
  return { ...progress, count: progress.resolved };
}

function transitionSessionTask(date, taskId, transition) {
  const entity = state.sessions?.[date];
  const current = entity?.taskStates?.[taskId];
  if (!entity || !current) throw new Error("The dated session task is unavailable");
  const next = Domain.transitionTaskState(current, transition);
  state.sessions[date] = {
    ...entity,
    taskStates: { ...entity.taskStates, [taskId]: next }
  };
  return next;
}
...
function renderCloudRecoveryShell() {
  return `<div class="onboarding-shell"><main class="auth-card card"><img class="auth-mark" src="/mark.svg" alt=""><p class="eyebrow">Pitching OS</p><h1>Your saved workspace needs attention.</h1><p class="auth-copy">Your encrypted cloud record is still stored. Pitching OS will not replace it with a blank workspace.</p><div class="alert warn"><strong>Cloud restore issue</strong>${esc(cloudSync.message || "The saved workspace could not be opened safely.")}<button class="text-button" data-action="retry-connection">Try again</button></div><p class="fineprint">Do not create a new workspace. Retrying is safe and does not delete the stored record.</p></main></div>`;
}

function renderShell() {
  if (accountAuth.loading || !accountAuth.signedIn) return renderAccountShell();
  if (accountAuth.workspaceReady && cloudSync.key && cloudSync.status === "error") return renderCloudRecoveryShell();
  ...
  return `
    ...
    ${activeModifyTask ? renderTaskModifyModal(activeModifyTask, selectedDate()) : ""}
  `;
}
...
function renderDashboard() {
  ...
  return `
    ...
    <section class="grid metrics today-shortcuts" aria-label="Today at a glance">
      <button class="card metric metric-shortcut ${pre?.risk === "green" ? "good" : pre?.risk === "yellow" || pre?.risk === "orange" ? "warn" : "accent"}" data-action="open-selected-session"><span class="metric-label">Provisional readiness</span><span class="metric-value">${pre ? (pre.score ?? "Hold") : "—"}</span><span class="metric-detail">${pre ? (pre.score === null ? "Safety override active" : `${pre.score}/100 provisional heuristic`) : "Complete today’s check-in"}</span>${dataSourceTag(wearableLabel, healthSources.oura?.data || healthSources.appleHealth?.data ? "sensor" : "manual")}<span class="metric-arrow" aria-hidden="true">›</span></button>
  ...
  return `
    <article class="card gate">
      ...
      <form id="pre-form" class="form-grid" data-date="${date}">
        ...
        <div class="field warning-field"><label for="painfulMovement">Painful movement or altered mechanics?</label><select id="painfulMovement" name="painfulMovement" required><option value="">Select…</option><option value="no" ${values.painfulMovement === "no" ? "selected" : ""}>No</option><option value="yes" ${values.painfulMovement === "yes" ? "selected" : ""}>Yes — hold today’s plan</option></select><small>Choose yes if movement is painful or pain is changing how you throw, run or lift.</small></div>
      ...
      <p class="fineprint"><strong>What this does:</strong> Pitching OS applies the documented <strong>provisional readiness heuristic</strong> and returns 100%, 75%, 50% or a safety hold. A reported painful movement, warning sign, illness, shoulder/elbow pain of 5/10 or more, or a clinically meaningful two-point pain increase overrides the composite score. Personal baselines require at least five prior check-ins and use a rolling median. This is not diagnosis, injury prediction or medical clearance. Data is saved on this device${cloudSync.key ? " and encrypted before cloud autosave." : "; cross-device access requires encrypted cloud autosave."}</p>
    </article>
  `;
}
...
function renderTasks(session, date) {
  const taskStates = state.sessions?.[date]?.taskStates || {};
  const stages = [...new Set(session.tasks.map((item) => item.stage))];
  const statusFor = (item) => taskStates[item.id]?.status || "not-attempted";
  const isResolved = (item) => statusFor(item) !== "not-attempted";
  ...
  return stages.map((stage) => {
    ...
    const first = tasks[0];
    const completedCount = tasks.filter((item) => statusFor(item) === "completed").length;
    const modifiedCount = tasks.filter((item) => statusFor(item) === "modified").length;
    const skippedCount = tasks.filter((item) => statusFor(item) === "skipped").length;
    const resolvedCount = completedCount + modifiedCount + skippedCount;
    const stageComplete = resolvedCount === tasks.length;
    const stageOpen = stage === openStage;
    const progressLabel = modifiedCount ? `${modifiedCount} modified` : skippedCount ? `${skippedCount} skipped` : stageComplete ? "Complete" : "done";
    return `<details class="card task-stage ${stageComplete ? "stage-complete" : stageOpen ? "stage-current" : ""}" data-plan-stage="${stage}" ${stageOpen ? "open" : ""}><summary class="stage-head"><span class="stage-number">${stage}</span><span class="stage-copy"><strong>${esc(first.stageTitle)}</strong><small>${esc(first.stageDescription)}</small></span><span class="stage-progress" aria-label="${resolvedCount} of ${tasks.length} tasks resolved"><strong>${resolvedCount}/${tasks.length}</strong><small>${esc(progressLabel)}</small></span><span class="stage-chevron" aria-hidden="true"></span></summary><div class="task-list">${tasks.map((item) => {
      const taskState = taskStates[item.id] || Domain.createTaskState(item.id);
      const done = taskState.status === "completed" || taskState.status === "modified";
      const skipped = taskState.status === "skipped";
      const modified = taskState.status === "modified";
      const canChange = item.stageTitle !== "Health Hold";
      const stateNote = skipped
        ? `<p class="task-skip-note"><strong>${esc(taskState.reason)}</strong>${taskState.notes ? ` · ${esc(taskState.notes)}` : ""}</p>`
        : modified
          ? `<p class="task-skip-note"><strong>Modified · ${esc(taskState.reason)}</strong> · ${esc(taskState.modification)}${taskState.notes ? ` · ${esc(taskState.notes)}` : ""}</p>`
          : "";
      return `<article class="task ${done ? "completed" : ""} ${skipped ? "skipped" : ""} ${modified ? "modified" : ""} ${item.adapted ? "task-adapted" : ""}" data-task-row="${item.id}"><input class="task-check" type="checkbox" aria-label="${skipped ? "Skipped" : "Complete"} ${esc(item.name)}" data-action="toggle-task" data-task="${item.id}" ${done ? "checked" : ""} ${skipped ? "disabled" : ""}><div><div class="task-title">${esc(item.name)}${item.adapted ? `<span class="adaptive-badge">Adjusted</span>` : ""}${modified ? `<span class="adaptive-badge">Modified</span>` : ""}${skipped ? `<span class="skip-badge">Skipped</span>` : ""}</div><div class="task-prescription">${esc(item.prescription)}</div><p class="task-cue">${esc(item.cue)}</p>${stateNote}</div><div class="task-actions"><button class="task-details" data-action="task-details" data-task="${item.id}">Details</button>${canChange && !skipped ? `<button class="task-skip" data-action="modify-task" data-task="${item.id}">${modified ? "Edit modification" : "Modify"}</button>` : ""}${skipped ? `<button class="task-skip undo" data-action="undo-skip-task" data-task="${item.id}">Undo skip</button>` : canChange && !done ? `<button class="task-skip" data-action="skip-task" data-task="${item.id}">Skip</button>` : ""}</div></article>`;
    }).join("")}</div></details>`;
  }).join("");
}
...
function renderPostForm(date, post = null, progress = null) {
  ...
  const skippedCount = Number(progress?.skipped ?? Domain.taskProgress(state.sessions?.[date]?.taskStates || {}).skipped);
  ...
  return `
    <article class="card gate" id="post-card">
      <div class="gate-icon">✓</div><h3>Plan resolved—check out</h3><p>Record what actually happened.${skippedCount ? ` ${skippedCount} task${skippedCount === 1 ? " was" : "s were"} skipped and will remain separate from completed work.` : ""} PULSE fields are optional and remain separate from the provisional manual throwing-load index.</p>
      <form id="post-form" class="form-grid" data-date="${date}">
        ...
        <div class="field warning-field"><label for="postPainfulMovement">Painful movement or pain-altered mechanics?</label><select id="postPainfulMovement" name="postPainfulMovement" required><option value="">Select…</option><option value="no" ${post?.postPainfulMovement !== "yes" ? "selected" : ""}>No</option><option value="yes" ${post?.postPainfulMovement === "yes" ? "selected" : ""}>Yes — apply safety hold</option></select><small>This safety result overrides the next composite planning display.</small></div>
    ...
    ${!pre ? renderPreForm(date) : `
      <div class="session-layout">
        ...
        <aside class="sticky-panel">
          <article class="card readiness-card ${pre.risk}"><div class="readiness-score"><div class="score-orb">${pre.score ?? "Hold"}</div><div><span class="status ${pre.risk}">${pre.manualOverride?.active ? "manual 100%" : pre.planLevel || pre.risk}</span><p>Provisional readiness heuristic · not medical clearance</p></div></div><p class="readiness-scope">Applies to throwing, plyos, gym, speed and conditioning. Safety overrides take priority over this composite display.</p></article>
...
function renderTaskModifyModal(item, date) {
  const taskState = state.sessions?.[date]?.taskStates?.[item.id] || Domain.createTaskState(item.id);
  return `<div class="modal-backdrop" data-action="close-modal"><article class="modal skip-modal" role="dialog" aria-modal="true" aria-labelledby="modify-modal-title" data-modal><header class="modal-head"><div><p class="eyebrow">Daily plan</p><h2 id="modify-modal-title">Modify ${esc(item.name)}</h2><p>Record what changed so the plan and completed work stay distinct.</p></div><button class="modal-close" data-action="close-modal" aria-label="Close">×</button></header><div class="modal-body"><form id="task-modify-form" class="form-grid" data-date="${esc(date)}" data-task="${esc(item.id)}"><div class="field full"><label for="taskModifyReason">Reason</label><select id="taskModifyReason" name="reason" required><option value="">Select a reason…</option>${["Readiness adjustment", "Pain-free movement substitution", "Coach or clinician direction", "Equipment unavailable", "Time constraint", "Other recorded reason"].map((reason) => `<option value="${esc(reason)}" ${taskState.reason === reason ? "selected" : ""}>${esc(reason)}</option>`).join("")}</select></div><div class="field full"><label for="taskModification">What was performed instead?</label><textarea id="taskModification" name="modification" maxlength="500" required placeholder="Example: 2 × 5 at 100 kg instead of 4 × 5 at 120 kg.">${esc(taskState.modification)}</textarea></div><div class="field full"><label for="taskModifyNotes">Optional note</label><textarea id="taskModifyNotes" name="notes" maxlength="500">${esc(taskState.notes)}</textarea></div><p class="fineprint field full"><strong>What this means:</strong> the task counts as performed and remains labelled Modified. The original prescription is preserved beside your recorded change.</p><div class="form-actions"><button class="btn btn-outline" type="button" data-action="close-modal">Cancel</button><button class="btn btn-dark" type="submit">Save modification</button></div></form></div></article></div>`;
}
...
function renderAnalyticsPage() {
  ...
  const avgReadinessValues = entries.filter((item) => item.pre?.score !== null && Number.isFinite(Number(item.pre?.score))).map((item) => Number(item.pre.score));
  ...
  return `
    <section class="page-head"><div><p class="eyebrow">Logged, not guessed</p><h2>See the pattern.</h2><p>PULSE fields show only values you enter from the sensor. Session-RPE and the provisional manual throwing-load index are clearly labelled calculations.</p></div><button class="btn btn-outline" data-action="export">Export data</button></section>
    ...
    <section class="grid metrics">
      <article class="card metric good"><span class="metric-label">Average provisional readiness</span><div class="metric-value">${avgReadiness || "—"}</div><div class="metric-detail">Unvalidated heuristic · ${avgReadinessValues.length} scored check-ins</div></article>
    ...
    <section class="grid two">
      <article class="card card-pad"><div class="card-head"><div><h3>Provisional readiness trend</h3><p>Pitching OS questionnaire and available wearable inputs</p></div></div>${lineChart(entries.filter((item) => item.pre?.score !== null), (item) => item.pre.score, "var(--blue)", "Provisional readiness heuristic")}<p class="fineprint">This 0–100 composite is provisional and unvalidated. Safety holds are not converted to zero and are excluded from the chart. It is not diagnosis, medical clearance or injury prediction.</p></article>
...
async function handleClick(event) {
  ...
  if (action === "toggle-task") {
    ...
    const occurredAt = new Date().toISOString();
    transitionSessionTask(date, taskId, {
      status: target.checked ? "completed" : "not-attempted",
      updatedAt: occurredAt
    });
  ...
  if (action === "modify-task") {
    const item = currentSessionTask(target.dataset.task);
    if (!item || item.stageTitle === "Health Hold") return;
    activePlanStage = { date: selectedDate(), stage: item.stage };
    activeModalTask = null;
    activeSkipTask = null;
    activeModifyTask = item;
    render(true);
    return;
  }
  if (action === "undo-skip-task") {
    ...
    const item = currentSessionTask(taskId);
    if (item) activePlanStage = { date, stage: item.stage };
    const skip = state.sessions?.[date]?.taskStates?.[taskId];
    if (skip?.status !== "skipped") return;
    const occurredAt = new Date().toISOString();
    transitionSessionTask(date, taskId, { status: "not-attempted", updatedAt: occurredAt });
    delete state.post[date];
  ...
  if (action === "remove-readiness-override") {
    ...
    delete state.post[date];
    delete state.editingPost;
    assignDatedSession(date, "plan_reassigned_after_override_removed");
  ...
  if (action === "remove-profile-photo") {
    ...
    saveState({ label: "Athlete profile" });
    render(true);
  ...
  if (action === "export") {
    const validated = Domain.parseAppStateV2(state);
    const blob = new Blob([JSON.stringify({ exportedAt: new Date().toISOString(), data: validated }, null, 2)], { type: "application/json" });
...
async function handleSubmit(event) {
  ...
  if (form.id === "pre-form") {
    ...
    delete state.sessions[date];
    delete state.post[date];
    assignDatedSession(date, "plan_assigned_after_check_in");
    saveState();
    render();
    const planMessages = {
      hold: "Health hold applied. Training was replaced with recovery guidance.",
      recovery: `Provisional readiness ${readiness.score}/100 · session changed to a recovery-modified workload.`,
      reduced: `Provisional readiness ${readiness.score}/100 · session volume reduced to about 75%.`,
      full: `Provisional readiness ${readiness.score}/100 · full session available.`
    };
  ...
  if (form.id === "task-modify-form") {
    ...
    if (!item || item.stageTitle === "Health Hold") {
      activeModifyTask = null;
      render(true);
      showToast("That task cannot be modified.");
      return;
    }
    const occurredAt = new Date().toISOString();
    transitionSessionTask(date, taskId, {
    ...
    delete state.post[date];
    delete state.editingPost;
    appendTrainingHistory("taskChanges", date, "task_modified", {
      runId: state.pre[date]?.runId || "",
      taskId,
      taskName: item.name,
      reason: values.reason,
    ...
    saveState({ label: "Session tasks" });
    render(true);
    showToast("Task modification saved with the original prescription preserved.");
    return;
  }
  ...
  if (form.id === "readiness-override-form") {
    ...
    const date = form.dataset.date;
    const pre = state.pre[date];
    if (!pre || !["reduced", "recovery"].includes(pre.planLevel) || pre.safety?.override || pre.risk === "red") {
    ...
    delete state.post[date];
    delete state.editingPost;
    assignDatedSession(date, "plan_reassigned_after_manual_override");
    saveState({ label: "Plan adjustment" });
    render();
  ...
  if (form.id === "post-form") {
    ...
    const occurredAt = new Date().toISOString();
    const datedSession = state.sessions?.[date];
    const taskStates = Object.values(datedSession?.taskStates || {});
    const taskById = Object.fromEntries((datedSession?.plan?.tasks || []).map((task) => [task.id, task]));
    const skippedTaskRecords = taskStates.filter((task) => task.status === "skipped").map((task) => ({ ...task, taskName: taskById[task.taskId]?.name || task.taskId }));
    const modifiedTaskRecords = taskStates.filter((task) => task.status === "modified").map((task) => ({ ...task, taskName: taskById[task.taskId]?.name || task.taskId }));
    const completedTaskIds = taskStates.filter((task) => task.status === "completed" || task.status === "modified").map((task) => task.taskId);
    const provisionalLoad = Domain.calculateProvisionalThrowingLoad({ low, moderate, high, game });
    const postSafety = Domain.evaluateSafety({
      shoulder: Number(values.postShoulder),
      elbow: Number(values.postElbow),
      painfulMovement: values.postPainfulMovement || "no"
    }, {
      shoulder: Number(state.pre[date]?.shoulder || 0),
      elbow: Number(state.pre[date]?.elbow || 0)
    });
    const postRecord = {
      ...values,
      lowThrows: low,
      ...
      estimatedLoad: provisionalLoad.index,
      estimatedLoadFormulaId: provisionalLoad.formulaId,
      estimatedLoadLabel: provisionalLoad.label,
      srpe: duration * rpe,
      ...
      modifiedTaskCount: modifiedTaskRecords.length,
      modifiedTasks: modifiedTaskRecords.map((item) => ({ ...item })),
      safety: postSafety,
    ...
    if (pbResult.updates.length) {
      ...
      showToast(postSafety.override ? "Session saved. If the elevated pain remains at the next check-in, the safety hold will override the composite score." : "Session saved and analytics updated.");
    }
    return;
  }
  ...
  if (form.id === "profile-form") {
    const values = formValues(form);
    state.profile = { ...state.profile, ...values, height: Number(values.height), weight: Number(values.weight), updatedAt: new Date().toISOString() };
    saveState({ label: "Athlete profile" });
    render(true);
...
async function handleChange(event) {
  ...
  if (event.target.id === "import-file" && event.target.files?.[0]) {
    try {
      const incoming = Domain.parseBackupJson(await event.target.files[0].text());
      const nextState = hydrateState(incoming);
      Domain.parseAppStateV2(nextState);
      state = nextState;
      state.page = "profile";
      saveState();
      render();
      ...
      showToast(error.message || "That file is not a valid Pitching OS backup.");
    }
  }
}
...
document.addEventListener("keydown", (event) => {
  if (event.key === "Escape" && (activeModalTask || activeSkipTask || activeModifyTask)) {
    activeModalTask = null;
    activeSkipTask = null;
    activeModifyTask = null;
    render(true);
